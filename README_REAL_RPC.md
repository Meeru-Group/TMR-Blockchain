...
For quick testing in a normal browser, `/rpc` also accepts read-only GET requests.
These helpers do not submit transactions and do not create blocks.
...
- `/rpc?method=tmr_getBlockByNumber&height=1`
- `/rpc?method=tmr_getBlockByHash&hash=<block-hash>`
- `/rpc?method=tmr_getTransactionByHash&hash=<tx-hash>`
- `/rpc?method=tmr_getBalance&address=<tmr-address>`

Standard JSON-RPC 2.0 remains POST-only and should be used by wallets and bridge software.
